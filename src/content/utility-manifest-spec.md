...
transform:
  pipe:
    - lines                          # bare string = a no-arg primitive
    - filter: { contains: "ERROR" }  # one-key map  = a primitive + parameter
    - replace: { from: '\[(.*?)\] ERROR (\S+): (.*)', to: '$1,$2,$3' }
    - table
```
...
| Group | Primitives |
|---|---|
| structure | `lines`, `split: <sep>`, `join: <sep>` |
| list | `sort`, `dedupe`, `reverse`, `filter: { contains \| not_contains \| matches \| not_matches }`, `head: N`, `tail: N`, `slice: { from, to }` |
| string | `trim`, `upper`, `lower`, `title`, `replace: { from, to }`, `prefix: <s>`, `suffix: <s>`, `template: <s>`, `sanitize`, `camel`†, `snake`†, `kebab`†, `slug`† |
...
recognize:
  patterns:
    - regex: '"incident_id"\s*:\s*"INC'   # JS-dialect; use `flags`, not inline (?i)
      flags: i                            # optional subset of i/m/s/u
  entity: Incident                        # optional; card entity kind (defaults to `emits`)
  confidence: ambient                     # ambient (default) | high
  anchors: { min_len: 8, require: ["INC"] }  # optional false-positive gates
...
```yaml
recognize:
  patterns: [ { regex: '\bINC\d+\b' } ]
  reps:
    - { label: Slack, template: "🔴 {value}" }
...
recognize:                         # claim a pasted incident JSON
