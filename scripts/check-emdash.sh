...
#
# The one exclusion is inside <style>/<script> blocks. Those carry ~156 dashes
# in CSS comments from the deck templates. They are worth cleaning, but they
# are a separate batch and a stylesheet comment should not block a deploy.
#
...
find public/decks src/pages src/fragments -type f \( -name '*.html' -o -name '*.astro' \) \
...
src = re.sub(r"<(?:script|style)\b[^>]*/>", blank, src)
src = re.sub(r"<(script|style)\b[^>]*>[\s\S]*?</\1>", blank, src)
